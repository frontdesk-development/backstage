...
export const PluginHeader: FC<Props> = ({ title = 'CloudBuild' }) => {
  const [, { showSettings }] = useSettings();
  const location = useLocation();
  const notRoot = !location.pathname.match(/\/cloudbuild\/?$/);
  const isSettingsPage = location.pathname.match(/\/cloudbuild\/settings\/?/);
  return (
    <ContentHeader
      title={title}
      titleComponent={() => (
        <Box alignItems="center" display="flex">
          {notRoot && (
            <IconButton component={RouterLink} to="/cloudbuild">
              <ArrowBack />
            </IconButton>
          )}
